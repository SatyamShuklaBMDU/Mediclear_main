...
<a class="btn btn-primary ml-2" href="{{ url('/corporate-vertigo-report') }}" role="button">Go Back</a>
<div class="container mt-5">
    <input type="hidden" id="consumer_id" value="{{ request()->get('id') }}" />

    @php

        $corporateBatch = \App\Models\CorporateBatch::select(
            'company.name as company_name',

            'corporatebatchs.batch_no as company_batch_no',
        )
            ->join('company', 'corporatebatchs.company_id', '=', 'company.id')
            ->where('corporatebatchs.id', request()->get('id'))
            ->get();

        $corporatedata = json_decode($corporateBatch)[0];
    ...
    <h1>Company Name<small class="text-muted">---{{ $corporatedata->company_name }}</small></h1>
    <h1>Batch No<small class="text-muted">---{{ $corporatedata->company_batch_no }}</small></h1>
...
<script>
    $(function() {
        ...
        var table = $('.data-table').DataTable({
            ...
            ajax: {
                url: "{{ route('consumer-corporate-vertigo-report') }}",
                type: "post",
                data: {
                    _token: csrfToken,
                    "id": id
